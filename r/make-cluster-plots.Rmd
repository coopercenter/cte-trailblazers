...
for (clust in cluster_names_vec){
  ...
  saveRDS(edu_plot, here::here("ggplots", paste0(cluster_3ltr,"-edu-ggplot.Rds")))
...
```{r write functions to make job growth plots}

clust <- cluster_names_vec[1]
...
job_growth_plot <- ggplot(linked_pathway, aes(value, pathway_for_labels_change, fill =variable)) +
 ...
 # saveRDS(job_growth_plot, here::here("ggplots", paste0(cluster_3ltr,"-job-growth-ggplot.Rds")))
...
```{r}
readRDS(here::here("ggplots", paste0(cluster_3ltr,"-job-growth-ggplot.Rds")))
```
...
```{r make job growth plots Biz Admin modified}
...
```{r make wage + openings + growth plots, warning = FALSE, eval = FALSE}

my_colors3 <- c("#1170aa", "#fc7d0d", "#003366", "#5fa2ce", "#c85200", "#a3cce9", "#ffbc79")

for (x in cluster_names_vec){
