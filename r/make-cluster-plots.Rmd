...
cluster_list <- unique(nonduplicated_prepped$cluster, fromLast = TRUE) 
cluster_list <- cluster_list[which(cluster_list != "All Occupations")]
...
for (x in cluster_list){
  ...
  edu_graph <- nonduplicated %>% 
                 filter(cluster == x, !is.na(predominant_education_level)) %>% 
  ...
  cluster_3ltr <- strtrim(x,3)
  
  saveRDS(edu_graph, here::here("ggplots", paste0(cluster_3ltr,"-edu-ggplot.Rds")))
...
```{r}
# Ene_edu_ggp <- readRDS(here::here("ggplots", paste0(cluster_3ltr,"-edu-ggplot.Rds")))

readRDS(here::here("ggplots", paste0("Fin","-edu-ggplot.Rds")))
```
