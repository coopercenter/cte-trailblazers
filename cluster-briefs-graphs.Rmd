...
my_colors <- c("#003366", "#2E5882", "#7490AC", "#C4D0DC")
...
ggplot(ag_linked_pathway, aes(value, pathway_for_labels_change, fill =variable)) +
  ...
  scale_fill_manual(values = my_colors2,
                    labels = jobs_labels) +
  scale_x_continuous(labels = comma, expand = expansion(mult = c(0, .1))) +
...
for (x in cluster_list){
   income_graph2 <- nonduplicated %>% filter(cluster == x, !is.na(pathway), !is.na(median_income)) %>% 
     ggplot(aes(median_income, pathway, color = pathway, size = annual_openings)) + 
     geom_point() + 
     scale_color_manual(values = my_colors3) + 
     scale_x_continuous(labels = scales::dollar_format()) + 
     xlab("Median Income") + 
